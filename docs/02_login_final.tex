...
\subsection{Scenáre a výsledky}
\begin{tabularx}{\textwidth}{|>{\bfseries}l|X|>{\bfseries}l|>{\bfseries}l|}
    \hline
    ID scenára & Popis kroku & Výsledok & Návrh riešenia \\
    \hline
    \textit{T-01} & \textit{napr. registrácia a prihlásenie používateľa} & \textit{OK/Zlyhalo} & \textit{oprava, ticket, dôvod} \\
    \hline
    \textit{T-02} & \textit{napr. prechod medzi hlavnými obrazovkami} & \textit{OK/Zlyhalo} & \textit{návrh fixu alebo potvrdenie stability} \\
    \hline
    \textit{T-03} & \textit{napr. stabilita minihry pri opakovaní} & \textit{OK/Zlyhalo} & \textit{identifikovaný bug alebo nie} \\
    \hline
\end{tabularx}
...
\section{Záver}
\begin{itemize}
    \item Zhrnutie stavu projektu pred odovzdaním: \textit{hlavné prínosy, stabilita, známe limity}.
    \item Odporúčania pre ďalší rozvoj: \textit{čo by sa hodilo dopracovať po odovzdaní}.
\end{itemize}
